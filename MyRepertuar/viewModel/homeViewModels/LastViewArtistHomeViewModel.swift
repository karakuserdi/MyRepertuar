//
//  LastViewArtistHomeViewModel.swift
//  MyRepertuar
//
...
struct LastViewArtistHomeViewModel{
    let lvArtist:SanatciData
    
    var artistName: String?{
        return lvArtist.sanatciAdi
    }
    ...
    var colorName: String?{
        ...
        let firstChar = Array(lvArtist.sanatciAdi)[0]
        let rowColor = colors[(letters.firstIndex(of: "\(firstChar)") ?? 0) % colors.count]
        return rowColor
    }
    
    init(lvArsits:SanatciData){
        self.lvArtist = lvArsits
    }
}
